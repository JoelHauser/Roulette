...
the number that paid.
    ...
    final = turns * 360 + (Position / PocketCount) * 360
    angle = final * ease(t)          // ease out, ~5s, ball counter-rotating

**Draw the wheel from `Wheel.Pockets` rather than from an image.** Poker lost real
time to a table photograph whose cloth was not centred in the picture and did not fill
it: everything placed against the felt *rect* was placed against the picture instead,
and the fix needed the image measured pixel by pixel. A wheel drawn from the pocket
list cannot disagree with the pocket list, and the pocket the marker points at is the
pocket that won, by construction. If an image is used later it needs a measured offset
for where its zero sits and a test that the two orders match.
...
- Repo initialised on `main`. No remote yet.
- **`Roulette.Game` is done and green at 70 tests**, in about 50ms. European and
  American wheels, the full cloth, every bet, exact payouts, the caps, and settlement.
- **Nothing else exists.** No server project, no client, no console, no scripts -- the
  folders are there and empty.

### Open items

**Next**

- **The server.** Port `Bank`, `ProfileGateway`, `Escrow`, `Wallets`, `Abstractions`,
  `ModMetadata`, `Startup` and the log from Poker. Then `RouletteService` with
  place / clear / spin / state, both routers, and the contracts.
...
- **The client, and the spinning wheel.** Port `Textures`, `ChipView`, `EscapePatch`,
  `TaskBarTab`, `MenuIcon` and `ProfileSync` from Poker; the chip art carries over
  whole. The wheel and the cloth are new.
- **The console harness**, so the wheel can be spun thousands of times without Unity.

**Decisions left open**

- **Which wheel ships by default.** European, on the grounds that there is no reason
  to hand a single player the worse of the two. `RouletteRules.Wheel` switches it and
  nothing branches on it except the top line, so this is a rules change and not a
  rewrite.
- **Whether the chips are the same six denominations as Poker's.** The art is there
  and `MinBet` is 10,000 to match it. Roulette wants smaller chips than poker does --
  a straight-up bet at the minimum already returns 360,000.
